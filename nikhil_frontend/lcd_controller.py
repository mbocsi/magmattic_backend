...
class LCDController(LCDInterface):
    ...
    async def initialize_display(self) -> None:
        ...
        if hasattr(cfg, 'POWER_SWITCH'):
            GPIO.setup(cfg.POWER_SWITCH, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        ...
        # Setup button callbacks with debouncing
        ...
        if hasattr(cfg, 'POWER_SWITCH'):
            GPIO.add_event_detect(
                cfg.POWER_SWITCH,
                GPIO.FALLING,
                callback=lambda x: asyncio.create_task(self.toggle_power()),
                bouncetime=300,
            )
